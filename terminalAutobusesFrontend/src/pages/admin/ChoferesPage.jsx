...
import { getChoferes, createChofer, updateChofer, deleteChofer } from '../../services/adminApi'
...
export default function ChoferesPage() {
  ...
  const fetchData = useCallback((p = page) => {
    ...
    getChoferes({ page: p, size: 20, sort: 'id,asc' })
      .then((res) => {
  ...
  function handleSave(e) {
    ...
    const promise = editing ? updateChofer(editing.id, payload) : createChofer(payload)
    promise
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={openCreate} className="bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium px-4 py-2 rounded-lg cursor-pointer">+ Nuevo chofer</button>
      </div>
      ...
      : rows.length === 0 ? (
        <div className="text-center py-12">
          <p className="text-gray-500 mb-2">No hay choferes registrados</p>
          <button onClick={openCreate} className="text-blue-600 text-sm font-medium cursor-pointer">Crear el primer chofer</button>
        </div>
      ) : (
        <>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Editar chofer' : 'Nuevo chofer'}>
